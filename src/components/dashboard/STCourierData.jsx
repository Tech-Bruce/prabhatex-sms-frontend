...
import { Search, Loader2, Trash2, Package, MapPin, Calendar, Weight, IndianRupee, CheckSquare, Square, Hash, FileText, ArrowRightLeft, Layers, Navigation } from 'lucide-react';
...
const STCourierData = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/couriers/st`);
  ...
  const filteredData = data.filter(item =>
    ...
    (item.refNo && item.refNo.toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  const handleSendSMS = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/couriers/st/send-sms`, {
        method: "POST",
        headers: {
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm("Are you absolutely sure you want to delete ALL ST Courier records? This action cannot be undone.")) return;
    setIsDeletingAll(true);
    try {
      const res = await fetch(`${API_BASE_URL}/couriers/st/all`, {
        method: "DELETE",
      });
      const resData = await res.json();
      if (resData.success) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/couriers/st/delete-bulk`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/couriers/st/${id}`, {
        method: "DELETE",
      });
      const resData = await res.json();
      if (resData.success) {
  ...
  const getStatusStyle = (status) => {
    ...
    if (s.includes('pending') || s.includes('transit') || s.includes('process'))
      return 'bg-amber-50 text-amber-700 border-amber-200/60';
    if (s.includes('return') || s.includes('fail') || s.includes('cancel'))
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">ST Courier Data</h2>
          <p className="text-slate-500 text-sm mt-1">View all data extracted from ST Courier PDF uploads.</p>
        </div>
              ...
              >
                ...
                <div className="flex items-start justify-between p-6 pb-4">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-0.5">AWB No</p>
                      <p className="text-base font-bold text-slate-900 tracking-tight break-all">{item.awbNo || 'N/A'}</p>
                    </div>
                  </div>
                  <button
                ...
                <div className="px-6 pb-4 flex items-center gap-2 flex-wrap">
                  <div className="inline-flex items-center gap-1.5 bg-emerald-50 border border-emerald-200/60 text-emerald-700 px-3 py-1.5 rounded-lg">
                    ...
                    <span className="text-sm font-bold">{item.codAmount ? parseFloat(item.codAmount).toFixed(2) : '0.00'}</span>
                    <span className="text-xs text-emerald-500 font-medium ml-0.5">COD</span>
                  </div>
                  {item.status && (
                    <span className={`inline-flex items-center px-2.5 py-1.5 rounded-lg text-xs font-semibold border ${getStatusStyle(item.status)}`}>
                      {item.status}
                    </span>
                  )}
                </div>
                ...
                <div className="p-6 pt-4 space-y-3">
                  ...
                  {item.refNo && (
                    <div className="flex items-center gap-2.5 text-sm">
                      ...
                      <span className="text-slate-500 w-14 flex-shrink-0">Ref</span>
                      <span className="text-slate-800 font-medium break-words">{item.refNo}</span>
                    </div>
                  )}

                  <div className="flex items-start gap-2.5 text-sm">
                    <Navigation className="w-4 h-4 text-slate-400 flex-shrink-0 mt-0.5" />
                    <span className="text-slate-500 w-14 flex-shrink-0">Route</span>
                  ...
                  {item.city && (
                    <div className="flex items-start gap-2.5 text-sm">
                      ...
                      <span className="text-slate-500 w-14 flex-shrink-0">City</span>
                      <span className="text-slate-800 font-medium break-words">{item.city}</span>
                    </div>
                  )}
                  ...
                  {/* Reason - if present */}
                  {item.reason && (
                    <div className="mt-1 p-2.5 bg-amber-50/60 rounded-lg border border-amber-100">
